	...
	function addScripts( $print = false ) 
	{
		...
		jQuery(document).ready(function() {
			...
			if(is_numeric($this->id_timeline) || isset($this->customData)) {
					...
					foreach($timeline_feeds as $key) {
						...
						if($key->name == 'youtube') { 
							...
							foreach($yt_arr as $yt) {
								...
								$yt_string .= dpSocialTimeline_plugin_url('lib/youtube.php?username='.trim($yt));
							}
							$key->data = $yt_string; 
						}
						if($key->name == 'youtube_search') { 
							...
							foreach($yt_search_arr as $yt_search) {
								...
								$yt_search_string .= dpSocialTimeline_plugin_url('lib/youtube.php?q='.trim($yt_search));
							}
				...
				lang_less: "'.($this->timeline_obj->lang_less != "" ? $this->timeline_obj->lang_less : "less than one minute ago").'",
				';
			}
			$script .= '
